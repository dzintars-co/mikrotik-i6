(function () {
  ...
  panel.innerHTML = `
    ...
    <label>3️⃣ Tunelis uz mājām?</label>
  ...
  panel.querySelector("#agent-send").onclick = async () => {
    ...
    if (!email.includes("@")) {
      ...
      status.textContent = "❌ Ievadi derīgu e-pastu!";
      return;
    }

    status.style.display = "block";
    status.style.color = "#0b1220";
    status.textContent = "Sūta ziņu...";
    ...
    try {
      ...
      if (resp.ok) {
        ...
        status.textContent = "✅ Ziņa nosūtīta!";
        setTimeout(() => {
        ...
        status.style.color = "crimson";
        status.textContent = "❌ Neizdevās nosūtīt.";
      }
    } catch (err) {
      status.style.color = "crimson";
      status.textContent = "⚠️ Tīkla kļūda.";
    }
  };
})();
